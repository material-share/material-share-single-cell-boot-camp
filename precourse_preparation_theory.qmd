...
-   Finding genes that are active in certain cell types under certain conditions. Example**:** Finding genes that switch on in lung cells in asthma.
...
Before proceeding with Rhapsody workflow, sample of interest is processed to have **live** **single cells in cell suspension**. It is optional to sort cells to concentrate on cell type of interest.
...
In the next chapter, you will learn what can be measured on the single cell level with BD Rhapsody^™^ System.

## 2.1 BD Rhapsody^™^ Single Cell System

![](prerequisite_figures/BD_Rhapsody_assays_summary.png)

The BD Rhapsody™ Single Cell System enables simultaneous measurement of multiple cell parameters in thousands of individual cells. By combining available assays, researchers can generate robust and intricate datasets to unlock powerful insights.
...
**Sample multiplexing with antibody-based techniques (SMK):** Sample multiplexing kit (SMK) use generic antibody conjugated with unique barcodes to label cells from different samples. This enables pooling of multiple samples in one cartridge.

**Immune Repertoire Profiling:** Full length V(D)J assay profiles the diversity of T cell and/or B cell receptor repertoires. This is valuable for studying the immune response and identifying antigen-specific cells.

**Antigen-Specific T-cell Analysis:** Immudex's technology enables the profiling of T cells that recognize specific antigens such as viral epitopes or tumor-related antigens. This expands the offering of immune profiling, aiding vaccine development and immunotherapy research.
...
The goal of sequencing analysis is to transform text-based genomic data into numerical data, and we achieve this through a series of steps in the pipeline. The steps of the pipeline are listed in simplified graphics below.
...
**U**nique **m**olecular **i**dentifiers (UMIs) are used to quantify molecules expressed in a cell. Here's how it works:

-   **Bead structure:** UMIs are small, unique sequences of DNA that are added to the capture sequence of beads. Each bead contains tens of thousands of unique UMIs followed by the polyA capture sequence. Each UMI is specific to the molecule it attaches to and becomes its unique tag.
...
UMIs can help to differentiate PCR copies and we can quantify correctly the feature expression. After sequencing, we count the number of unique UMIs rather than the number of reads of a transcript as feature expression. UMIs help to:

-   **Remove PCR duplicates:** Because UMIs are unique to each original RNA molecule, any duplicates (multiple copies of the same RNA molecule) with the same UMI are likely the result of PCR amplification during library preparation.
...
-   **Quantify gene expression:** By counting the unique UMIs associated with each gene, one can measure the expression level of features in a cell. Genes with more UMIs are more active, while genes with fewer UMIs are less active.

In summary, UMIs are used to distinguish real feature molecules from PCR copy by providing a unique identifier for each original molecule.

### 2.3.3 Output Files from Primary Pipeline
...
The primary analysis output files are the input of secondary analysis. During the course, we will work with demo data that include mRNA, protein and sample tag information. You will get to know how to load the expression matrix in MEX format, how to perform quality control and other common steps in Chapter 1 - 9.

::: callout-important
The course will ONLY cover the secondary analysis.
:::

# Additional Information on Running Primary Pipeline
