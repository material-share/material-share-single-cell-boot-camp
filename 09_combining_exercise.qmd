...
The weighted nearest neighbor (WNN) is a statistical framework implemented in Seurat for the multimodal analysis. WNN requires pre-processing and dimentionality reduction on both assays upfront.
...
p2 <- DimPlot(combined, reduction = 'wnn.umap', group.by = 'predicted.celltype.l2', 
              label = TRUE, repel = TRUE, label.size = 2.5) + 
...
Plot CD3 (protein expression) and CD3E (gene expression) side by side.
...
# switch default assay to AbSeq to plot protein expression
DefaultAssay(combined) <- "AB"
p1 <- FeaturePlot(combined, "CD3-CD3E-AHS0033-pAbO", 
                  cols = c("lightgrey", "darkgreen"), 
                  reduction = 'wnn.umap')

# switch default to RNA to plot gene expression
DefaultAssay(combined) <- "RNA"
p2 <- FeaturePlot(combined, "CD3E", reduction = 'wnn.umap')
